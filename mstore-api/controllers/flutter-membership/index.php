    ...
    public function membership_register()
    {
        if (is_plugin_active('indeed-membership-pro/indeed-membership-pro.php')) {
            if (!class_exists('FlutterUserAddEdit')) {
                require_once(__DIR__ . '/flutter-user-add-edit.class.php');
            }
            $nonce = wp_create_nonce('ihc_user_add_edit_nonce');
            $json = file_get_contents('php://input');
            $_POST = json_decode($json, TRUE);
            $_POST['ihc_user_add_edit_nonce'] = $nonce;
            $_REQUEST['ihc_payment_gateway_radio'] = sanitize_text_field($_POST['ihc_payment_gateway_radio']);
            $_REQUEST['ihc_payment_gateway'] = sanitize_text_field($_POST['ihc_payment_gateway']);
            $args = array(
                'user_id' => false,
                'type' => 'create',
                'tos' => true,
